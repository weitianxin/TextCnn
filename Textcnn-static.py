...
def get_data():
    ...
    return x,y,length,vocab.vocabulary_._mapping
...
def train():
    x,y,length,vocab_dict=get_data()
    vocab_size = len(vocab_dict)
    embeddings = 2*np.random.rand(vocab_size,embedding_dim)-1#initial with U[-1,1]

    #load the pretrained word vectors
    filename= os.listdir("pretrained word vector")[0]
    model = {}
    for i, line in enumerate(open(filename,"rb",encoding="utf-8")):
        if i!=0:
            values = line.split()
            model[values[0]] = np.array(values[1:],dtype=np.float64)

    for k, v in vocab_dict.items():
        try:
            embeddings[v] = np.array(model[k], dtype=np.float64)
        except:
            pass#this means there are no such words in the pretrained dict,just random initial.
    ...
    with tf.name_scope("embed"):
        #load the pretrained word vectors
        W = tf.Variable(initial_value=embeddings,dtype=tf.float32)
